...
- 展示检查井、泵站和病害事件叠加
- 将关联建筑按楼层切片显示，并支持在主视图中直接选层
- 支持“当前层模型”查看，展示当前层内的房间与房间设备
- 联动展示节点状态、节点详情和纵断面剖面
- 节点点击、列表点击、剖面点击互相联动
- 楼层点击、房间点击和设备点击互相联动
...
- 当前层模型中的房间布局仍是前端基于房间集合做的轻量平面编排，不是 CAD/BIM 级真实房间轮廓
- 当前层模型中的部分设备为基于房间类型与现有设备上下文生成的展示层，不等同于完整设备台账
